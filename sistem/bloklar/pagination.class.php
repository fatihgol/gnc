...
/*
 * Pagination Class
 * Dane Gardow <dane.g87@gmail.com> tarafından yapılan Sayfalama sınıfı
 * << < 1 2 3 .... > >> gibi içeriği sayfalamayı sağlar kullanımı oldukça basittir.
 * 
 	// Parametreler (100-> Gösterilecek eleman sayısı, 10->Max kaç sayfa gösterilecek, array->Select içinde neler gösterilecek)
	$Pagination = new Pagination(100, 10, array(5, 10, 25, "All"), "table", "pageOff", "pageOn", "pageSelect", "pageErrors");
	echo $Pagination->display(); 				// << < 1 2 3 .... 4 5 > >> 'i göster
	echo $Pagination->displaySelectInterface();	// Bir sayfada kaç tane eleman gösterilsin; 5,10,25,50 gibi 
		
	$start = $Pagination->getEntryStart();
	$end   = $Pagination->getEntryEnd();
	$query = mysql_query("SELECT * FROM table LIMIT $start,$end");
	
	şeklinde kullanılabilir. CSS için pageON ve pageOFF class'ları şekillendirilmelidir. 
 * 
 * 
 ...
 * @license		Free
	...
	3. Call the display functions to output pagination links and display settings. Donít forget to use echo.
	...
	5. Thatís it! All the pagination is taken care of; the only thing left is implementing how the data is displayed.
	...
	Array
	An associative array that contains CSS style class names for pagination styling. pageOff and pageOn refer to navigation links that are currently being viewed (on) or not (off). Select refers to the styling of the <select> element, and errors to the classís error reporting display style.
	...
	*Important: This function sends headers (error-handling redirection)! This function must be called after setEntriesPerPage(). This function sets the current page and retrieves any GET information, if available (otherwise sets the current page to the first page).
	...
	Public
	No arguments
	*Important: This function sends headers (cookies). This function contains the logic which processes viewer-specified display settings using POST method form submission. Cookies are also set, which remember the viewerís choice.
...
class Pagination
{
	...
	*******************************************************/
	...
	const PAGE_GETVAR_NAME		  = "sayfa";					 // Name of GET variable name for page values (i.e., example.php?page=1)
